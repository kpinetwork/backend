...
resource "aws_api_gateway_resource" "make_data_public" {
  path_part   = "make_public"
  parent_id   = aws_api_gateway_resource.companies.id
  rest_api_id = aws_api_gateway_rest_api.api.id
}
# ----------------------------------------------------------------------------------------------------------------------
...
resource "aws_api_gateway_method" "make_data_public_method" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.make_data_public.id
  http_method   = "PUT"
  authorization = "CUSTOM"
  authorizer_id = aws_api_gateway_authorizer.kpi_authorizer.id
}
...
resource "aws_api_gateway_integration" "make_data_public_integration" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.make_data_public.id
  http_method             = aws_api_gateway_method.make_data_public_method.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.lambdas_functions_arn.make_data_public_lambda_function
}
# ----------------------------------------------------------------------------------------------------------------------
# API GATEWAY DOMAIN
